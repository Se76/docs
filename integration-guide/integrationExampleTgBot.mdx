...
<Info>
  The Breeze API handles user fund account creation automatically, eliminating
  the need for manual initialization flows. The new strategy-based approach
  allows depositing multiple assets into a single strategy with individual APY
  tracking per asset.
</Info>
...
```json
{
  ...
  "apy_per_asset": {
    ...
    "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB": 4.8,
    "jupSoLaHXQiZZTSfEWMTRRgpnyFm8f6sZdosWBjx93v": 7.2
  }
}
```
...
class BreezeBot {
  ...
  private async getStrategyInfo(): Promise<StrategyInfo | null> {
    try {
      const response = await fetch(
        `https://api.breeze.baby/strategy-info/${STRATEGY_ID}`,
        {
          method: 'GET',
          headers: {
  ...
  private async getBreezeBalances(
    userPublicKey: string
  ): Promise<BreezeBalancesResponse | null> {
    try {
      const response = await fetch(
  ...
  private convertFromTokenAmount(
    tokenAmount: bigint,
    tokenSymbol: string
  ): number {
    const decimals = TOKEN_DECIMALS[tokenSymbol] || 6;
  ...
  private async getUserAssetBalance(
    userPublicKey: string,
    mint: string
  ): Promise<number> {
    try {
...
<Accordion title="Transaction Failures">
  - Check user has sufficient SOL for transaction fees - Verify token account
  exists and has sufficient balance - Ensure transaction isn't expired - Check
  Solana network status
</Accordion>

<Accordion title="Balance Discrepancies">
  - API returns values in token units (raw amounts) - Ensure proper decimal
  conversion using `convertFromTokenAmount` - Check for pending transactions -
