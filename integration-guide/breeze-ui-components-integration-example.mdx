...
<Card
  ...
  icon="github"
  href="https://github.com/anagrambuild/breeze-integration-examples/tree/master/breeze-powered-website"
>
...
export const SUPPORTED_TOKENS: Record<string, AssetConfig> = Object.fromEntries(
  AVAILABLE_ASSETS.map((asset) => [asset.symbol, asset])
);
    ...
    if (data.data && Array.isArray(data.data)) {
      const convertedData = {
        ...
        data: data.data.map((item: any) => {
          ...
          return {
            ...item,
            total_position_value: fromApiAmount(
              item.total_position_value,
              decimals
            ),
            total_deposited_value: fromApiAmount(
              item.total_deposited_value,
              decimals
            ),
...
export async function POST(request: NextRequest) {
  ...
  const tokenConfig = AVAILABLE_ASSETS.find(
    (asset) => asset.mint === base_asset
  );
  ...
  const strategyId =
    ...
    process.env.NEXT_PUBLIC_BREEZE_STRATEGY_ID;
  const baseUrl = process.env.BREEZE_API_BASE_URL;
  ...
  const fetchBreezeBalance = useCallback(async (): Promise<number> => {
    ...
    try {
      const response = await fetch(
        `/api/balance?user_pubkey=${publicKey.toBase58()}`
      );
  ...
  const executeTransaction = useCallback(
    async (
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`Failed to fetch ${operationType} transaction`);
        ...
        toast.success(
          `${operationType === 'deposit' ? 'Deposit' : 'Withdrawal'} successful!`,
          { id: toastId }
        );
        await onSuccess();
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : `${operationType} failed`;
