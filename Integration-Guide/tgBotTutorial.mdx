...
<Info>
  Breeze Protocol automatically handles user fund account creation,
  significantly simplifying the integration process for developers.
</Info>
...
class ConversationalBreezeBot {
  ...
  private async showContextualHelp(chatId: number, originalText: string) {
    ...
    if (originalText.includes('deposit')) {
      ...
      helpMessage +=
        "• Say: 'Deposit 100 USDC' or 'Invest 50% of my balance'\n";
...
export class BreezeAPIClient {
  ...
  async getUserBalances(
    userPubkey: string
  ): Promise<BreezeUserBalancesResponse> {
...
export class TransactionManager {
  ...
  async processTransaction(
    ...
    try {
      ...
      console.log(
        `Transaction confirmed in ${confirmationTime}ms: ${signature}`
      );
...
export class MessageFormatter {
  ...
  static formatTransaction(
    ...
    return (
      `${icon} **${action}**: ${this.formatCurrency(amount)} ${asset}\n` +
  ...
  static async generateYieldReport(
    ...
    try {
      ...
      const totalEarned = data.reduce(
        (sum, entry) => sum + parseFloat(entry.yield_earned),
        0
      );
      const avgAPY =
        data.reduce((sum, entry) => sum + parseFloat(entry.apy), 0) /
        data.length;
      const bestDay = data.reduce((best, entry) =>
        parseFloat(entry.yield_earned) > parseFloat(best.yield_earned)
          ? entry
          : best
      );
...
**Performance Trend**: ${this.getTrendEmoji(data)}
      ...
      return MessageFormatter.formatError(
        'Failed to generate yield report',
        'Analytics'
      );
    }
  }
